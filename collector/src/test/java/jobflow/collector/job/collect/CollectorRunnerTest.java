...
class CollectorRunnerTest {
    ...
    @Test
    @DisplayName("설정된 source로 sitemap crawl을 실행한다")
    void run() {
        ...
        CollectorRunner runner = new CollectorRunner(properties, sitemapCrawlService);
        ...
        given(sitemapCrawlService.crawl(JobIngestionSource.ZIGHANG))
                .willReturn(new SitemapCrawlResult(
                        ...
                        List.of(new CrawlerUrlCandidate(
                                JobIngestionSource.ZIGHANG,
    ...
    void runWithDefaultSource() {
        CollectorRunnerProperties properties = new CollectorRunnerProperties(true, null, 0);
        CollectorRunner runner = new CollectorRunner(properties, sitemapCrawlService);
