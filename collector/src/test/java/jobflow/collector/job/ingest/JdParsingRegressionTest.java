...
import jobflow.collector.skill.JdPhraseTagMapping;
import jobflow.collector.skill.JdPhraseTagMappingRepository;
...
class JdParsingRegressionTest {
    ...
    @Test
    @DisplayName("collector 백엔드 JD에서 skill, experience tag, role을 함께 정규화한다")
    ...
    @Test
    @DisplayName("collector 플랫폼 JD에서 Kubernetes alias, monitoring tag, DEVOPS role을 함께 정규화한다")
    void normalizePlatformJdParsingFlow() {
        Skill kubernetes = Skill.create("Kubernetes", "kubernetes", SkillCategory.INFRA);
        ExperienceTagCode monitoring = ExperienceTagCode.create(
                "MONITORING",
                "모니터링",
                "모니터링과 장애 추적 경험"
        );
