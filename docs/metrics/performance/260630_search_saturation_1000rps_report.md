...
JFR은 backend JVM에 `-XX:StartFlightRecording`을 적용해 수집했다. JFR recording 자체가 CPU와 I/O 비용을 추가하므로, 아래 1100RPS JFR run은 성능 기준선으로 사용하지 않고 CPU hotspot 후보를 찾는 용도로만 사용한다.

| 항목 | 값 |
|---|---:|
| target RPS | 1100 |
| actual RPS | 657.01 |
| total requests | 85,127 |
| dropped iterations | 46,875 |
| p50 latency | 11,276.66ms |
| p95 latency | 15,538.01ms |
| p99 latency | 17,486.58ms |
| max latency | 23,142.66ms |
| HTTP failure rate | 0% |
| max VUs | 8,000 |

JFR steady recording은 startup class loading 영향을 줄이기 위해 `delay=60s`, `duration=150s`로 수집했다. 이 조건에서도 k6가 max VUs 8,000에 도달하고 dropped iterations가 발생했으므로, JFR run의 latency 수치는 clean probe보다 불리하게 해석해야 한다.

### JFR 주요 신호

| 영역 | 관찰 | 해석 |
|---|---|---|
| CPU | JVM user 평균 43.69%, host machine total 평균 82.98%, 최대 100% | saturation 구간에서 host CPU queueing이 발생 |
| CPU throttling | throttled slices 0, throttled time 0s | cgroup CPU throttling보다는 host CPU 경합과 요청 처리 비용 쪽에 가까움 |
| GC | total pause 1.94s, p95 pause 38.5ms, max pause 62.3ms | GC pause가 p95 15s latency의 직접 원인이라고 보기 어려움 |
| Hot methods | `ObjectInputStream`, Jackson UTF8 string write, Micrometer key/tag, hash map/concurrent map 연산 관측 | cache hit path의 역직렬화, 응답 직렬화, metric/observation overhead 후보 |
| Allocation | `HeapByteBuffer`, `JwtTokenizer`, Lettuce `ByteArrayCodec`, Redis `ByteUtils`, `ObjectInputStream`, Micrometer `Tag.of` 상위권 | Redis/JDK serialization, JWT parsing, Micrometer tag 생성이 allocation pressure에 기여 |
| Socket I/O | MySQL read/write event가 Redis보다 많이 관측됨 | cache hit workload에서도 auth/preflight/consumer/background path 또는 metric sampling에 DB I/O가 섞일 수 있어 분리 측정 필요 |
| Contention | Tomcat class loader/filter map/recycled processor 관련 lock 대기 관측 | 초고동시 요청에서 servlet container 내부 lock contention 가능성 |

따라서 다음 개선 후보는 Hikari pool 증설이 아니라 다음 순서로 좁힌다.

1. 부하 발생기 분리 후 1000RPS/1100RPS를 재측정해 JFR/k6/application CPU 경합을 분리한다.
2. cache hit path의 Redis value serializer를 점검한다. 현재 JFR에는 `ObjectInputStream`과 Redis byte buffer allocation이 반복적으로 잡히므로 JDK serialization 비용을 우선 의심한다.
3. 검색 응답을 최종 DTO 또는 summary payload 단위로 캐싱해 cache hit 이후 DTO mapping과 response serialization 비용을 줄일 수 있는지 검토한다.
4. JWT parsing과 Micrometer observation/tag 생성 비용이 검색 saturation에서 어느 정도인지, 인증 헤더 없는 public search run과 actuator metric tag cardinality를 분리 비교한다.
5. Tomcat thread를 더 늘리기보다 container lock contention과 active request queueing을 줄이는 방향으로 접근한다.
...
### 4. 다음 병목 후보는 servlet worker/CPU queueing과 cache-hit 처리 비용이다
...
- Redis cache value serialization/deserialization 비용 측정
- JWT parsing과 Micrometer observation/tag 생성 비용 분리 측정
...
- JFR steady recording에서 Redis/JDK deserialization, JSON serialization, JWT parsing, Micrometer observation/tag 생성이 cache-hit path의 CPU/allocation 후보로 확인됐다.
...
|---|---|---|---|
| Load generator 분리 재측정 | 외부 k6 또는 별도 EC2에서 1000/1100RPS 재측정 | 1000RPS 재현, dropped iterations 0, p95 300ms 이하, 1100RPS p95 재확인 | k6/application CPU 공유 영향 제거 |
...
| Auth/observability 비용 분리 | 인증 헤더 없는 public search run과 Micrometer tag 비용 비교 | JWT/Micrometer 비용 비중 확인 | JWT parsing, observation filter, tag 생성 |
...
다음 작업의 1차 목표는 load generator를 분리한 상태에서 1000RPS를 재현하고 1100RPS tail latency를 다시 확인하는 것이다. 그 다음 JFR에서 확인한 cache-hit path 후보 중 Redis/JDK serialization과 response serialization 비용을 먼저 낮춘 뒤 1,500RPS 안정 처리 여부를 확인한다. 1,500RPS에서 tail latency가 안정되면 250RPS 단위로 목표를 높여 실제 saturation cliff를 다시 찾는다. 3,000RPS는 단일 통합 EC2 기준이 아니라, backend와 부하 발생기를 분리하고 필요하면 backend replica 또는 인스턴스 스펙 조정을 포함한 구조 개선 목표로 둔다.
...
| 1100RPS steady JFR run JSON | `jobflow-server-env/artifacts/260630_search_saturation_jfr_1100_steady/260630_k6_es_cache_saturation_jfr_steady_200k_1100rps.json` |
| 1100RPS steady JFR file | `jobflow-server-env/artifacts/260630_search_saturation_jfr_1100_steady/260630_search_saturation_1100_steady.jfr` |
| 1100RPS steady JFR views | `jobflow-server-env/artifacts/260630_search_saturation_jfr_1100_steady/260630_search_saturation_1100_steady_jfr_views.txt` |
| 1100RPS steady JFR hot methods | `jobflow-server-env/artifacts/260630_search_saturation_jfr_1100_steady/260630_search_saturation_1100_steady_jfr_hot_methods.txt` |
| 1100RPS steady JFR allocation | `jobflow-server-env/artifacts/260630_search_saturation_jfr_1100_steady/260630_search_saturation_1100_steady_jfr_allocation_by_site.txt` |
...
- JFR steady run은 관측 부하 때문에 성능 기준선으로 쓰지 않고, CPU/allocation 후보를 찾는 자료로만 사용한다.
- JFR 기준 다음 개선 후보는 Redis/JDK serialization, response JSON serialization, JWT parsing, Micrometer observation/tag 생성 비용이다.
- 다음 개선은 load generator 분리, cache-hit path 최적화, auth/observability 비용 분리 측정 방향으로 진행한다.
