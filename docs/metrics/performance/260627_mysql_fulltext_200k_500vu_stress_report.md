...
> **Stop-rule**: threshold는 위반됐지만 스크립트에 `abortOnFail`이 설정되지 않아 테스트가 중단되지 않고 23분 전체를 완주했다. 이후 스크립트에 `abortOnFail: true`를 추가해 다음 테스트부터 stop-rule이 실제로 작동하도록 수정했다.

> **dropped_iterations**: 이번 실행에서는 `dropped_iterations`를 별도로 기록하지 않았다. k6 summary JSON 미저장(Docker volume 권한 오류)으로 확인 불가. 다음 테스트부터 runner 스크립트에 `chmod 777`을 추가해 JSON artifact가 정상 저장되도록 수정했다.
...
Raw JSON은 Docker volume mount UID/GID 불일치로 저장 실패. runner 스크립트에 `chmod 777 $ARTIFACT_DIR` 추가로 수정 완료. 이번 실행 결과는 터미널 summary와 Grafana 캡처로 대체.
