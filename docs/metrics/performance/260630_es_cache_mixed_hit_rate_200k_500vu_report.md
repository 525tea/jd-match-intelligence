...
> 검토 메모: 아래 결과는 mixed runner에 cold-cache reset과 실행별 long-tail keyspace 격리를 추가하기 전에 수집한 초기 측정값이다. 따라서 안정성, 처리량, latency 관찰값으로는 사용할 수 있지만, 70/50/30 ratio 간 Redis hit/miss 비율을 엄밀히 비교하는 최종 apples-to-apples 결과로 해석하지 않는다. ratio별 hit-rate 비교를 확정하려면 수정된 runner로 Redis cold cache 상태에서 재측정한다.
...
| error rate | 테스트 구간 0% |
| cache hit rate | 세 조건 모두 Grafana 기준 거의 100% 근처 유지. 단, runner 수정 전 측정이라 long-tail key 반복 및 ratio 간 warmed cache 영향이 포함될 수 있음 |
...
따라서 이번 테스트는 인기 키워드 반복 조회뿐 아니라 long-tail 검색어가 섞인 warmed mixed workload에서도 Redis cache 기반 검색 경로가 안정적으로 동작한다는 근거로 사용할 수 있다.

단, 이 측정은 cold-cache reset과 충분한 long-tail cardinality를 강제하기 전의 초기 결과다. 수정된 runner는 실행 전 Redis cache를 비우고, 실행별 `LONG_TAIL_RUN_ID`와 기본 1,000,000개 long-tail keyspace를 사용한다. 실제 운영 트래픽의 keyword cardinality, pagination, filtering 조건이 더 커질 경우에는 별도 workload로 추가 측정해야 한다.
...
- high cache-hit workload에서 확인한 Redis cache 적용 경로가 warmed mixed workload에서도 안정적으로 유지됐다.
- 70/50/30 hot traffic 조건 모두 500VU에서 error rate 0%로 완주했다.
- hot traffic과 long-tail traffic의 p95 차이가 작았다. 다만 수정 전 runner의 warmed cache 영향이 포함될 수 있어, ratio별 hit/miss 효과 비교는 cold-cache 재측정 결과로 확정한다.
