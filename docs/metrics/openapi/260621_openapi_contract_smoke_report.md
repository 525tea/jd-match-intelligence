...
Gateway 기준 OpenAPI 계약이 실제 문서에 반영됐는지 확인한다.
...
백엔드 OpenAPI 문서는 런타임에서 생성되므로, 백엔드/Gateway 코드 또는 API 계약을 수정한 뒤에는 먼저 재기동한다.
...
| Jobs | `/jobs` query parameter `page` | `true` |
| Jobs | `/jobs` query parameter `size` | `true` |
| Jobs | `/jobs` query parameter `status` | `true` |
| Jobs | `/jobs` query parameter `role` | `true` |
| Jobs | `/jobs` query parameter `careerLevel` | `true` |
| Jobs | `/jobs` query parameter `locationRegion` | `true` |
| Jobs | `/jobs` query parameter `remoteType` | `true` |
...
- Jobs 목록 API가 OpenAPI 문서에 반영됐다.
- Jobs 목록 API의 페이지네이션/필터 query parameter가 OpenAPI 문서에 반영됐다.
- UserJob 저장/무시 처리 API가 기존 계약에 유지됐다.
- UserJob 저장 취소/무시 취소 API가 OpenAPI 문서에 반영됐다.
- Application 상태 변경 이력 조회 API가 OpenAPI 문서에 반영됐다.
- 프론트와 스모크 스크립트가 참조할 공개 API 계약이 최신 상태로 갱신됐다.
