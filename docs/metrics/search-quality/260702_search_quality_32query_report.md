...
- 수동 audit label은 `performance/elasticsearch/search_quality_manual_audit_labels.csv` 파일로 분리해 평가 script 내부 hardcoding을 제거했다.
...
| manual audit label file | `performance/elasticsearch/search_quality_manual_audit_labels.csv` |
...
| `.NET 개발자` | special_token | `0.8405` | `0.9307` | `+0.0902` |
...
현재 32-query / 1,240건 corpus에서는 추가 boost 튜닝으로 평균 NDCG를 더 끌어올릴 여지가 있다. 그러나 남은 낮은 query는 대부분 query intent와 label 기준이 애매한 영역이므로, 이 단계에서 점수만 올리는 튜닝은 한정된 평가셋에 맞춘 과튜닝이 될 수 있다고 판단했다. 따라서 ranking weight 조정은 여기서 멈추고, 30~50개 query로 evaluation set을 확장하되 수동 audit label을 함께 관리해 평가 신뢰도를 높이는 방향을 우선했다. 이후 실제 채용 플랫폼 규모에 가까운 10만건 corpus를 목표로 데이터를 확장하고, 그 기준에서 검색 품질 개선을 이어갈 예정이다.
...
Manual audit label 파일을 명시적으로 지정할 경우:

```bash
BASE_URL=http://127.0.0.1:8080 LIMIT=10 FETCH_LIMIT=40 \
MANUAL_LABEL_FILE=performance/elasticsearch/search_quality_manual_audit_labels.csv \
RUN_LABEL=es-framework-aware-v4-32q-manual-audit-real1240 \
OUTPUT_FILE=artifacts/search-quality/260702_search_quality_32query_es_framework_aware_v4_manual_audit_rows.csv \
SUMMARY_FILE=artifacts/search-quality/260702_search_quality_32query_es_framework_aware_v4_manual_audit_summary.json \
python3 performance/elasticsearch/search_ndcg10_evaluate.py
