...
export function JobFlowScreens({ t, go, screen }) {
      ...
      return <Shell title="프로젝트" sub="프로젝트 분석 결과가 연결되면 추출 스킬, 경험 태그, 매칭 공고가 이 화면에 표시됩니다." actions={<ActionButton onClick={() => go('jobs')}>공고 먼저 보기</ActionButton>}>
        <div style={{ display: 'grid', gridTemplateColumns: narrow ? '1fr' : '1.05fr 0.95fr', gap: 18, alignItems: 'stretch' }}>
          <section style={{ ...tile, padding: 28 }}>
            ...
            <p style={{ color: muted, fontSize: 15, lineHeight: 1.7, margin: '14px 0 0' }}>현재 프론트는 실제 project inventory API 응답만 프로젝트 카드로 보여줍니다. 데모 계정 또는 GitHub 로그인 후 `VITE_DEMO_PROJECT_ID`/localStorage의 `jobflow.userProjectId`가 실제 사용자 프로젝트 ID를 가리키면 이 화면이 채워집니다.</p>
            <div style={{ marginTop: 18, display: 'grid', gap: 10 }}>
              ...
              <StatePanel type="forbidden" title="새 레포 분석 API는 아직 연결 전입니다" desc="프로젝트 생성·재분석 endpoint가 생기기 전까지는 화면에서 가짜 분석 성공처럼 이동하지 않습니다." secondary="공개 공고 탐색" />
            </div>
          </section>
  ...
  const projectNewScreen = () => <Shell title="새 레포 분석" sub="프로젝트 생성·재분석 API가 연결되면 GitHub 레포 URL을 등록하고 분석을 시작합니다. 현재는 실제 endpoint가 없으므로 입력 내용을 저장하거나 성공 처리하지 않습니다." actions={<ActionButton onClick={() => go('projects')}>프로젝트 목록</ActionButton>}>
    <div style={{ display: 'grid', gridTemplateColumns: narrow ? '1fr' : '1fr 0.86fr', gap: 18, alignItems: 'start' }}>
      <section style={{ ...tile, padding: 26 }}>
        ...
        <H>분석할 레포지토리</H>
        <Field label="프로젝트 이름" placeholder="프로젝트 이름을 입력하세요" />
        <Field label="GitHub 레포 URL" placeholder="GitHub 레포 URL을 입력하세요" />
        ...
        <div style={{ marginTop: 14, display: 'flex', flexWrap: 'wrap', gap: 8 }}><ActionButton primary onClick={() => notify('프로젝트 생성 API 연결 후 사용할 수 있어요.')}>분석 시작 준비</ActionButton><ActionButton onClick={() => go('projects')}>프로젝트 목록으로</ActionButton></div>
        <div style={{ marginTop: 14 }}><StatePanel type="empty" title="아직 백엔드 생성 API가 없습니다" desc="현재 구현된 백엔드 API는 기존 프로젝트 분석 결과 조회 중심입니다. 새 레포 등록/재분석 endpoint가 추가되면 이 화면을 실제 요청으로 연결합니다." /></div>
  ...
  const projectAnalysisScreen = () => {
    if (!JF.__userProjectId || !JF.projectList.length) {
      return <Shell title="프로젝트 분석" sub="프로젝트 분석 결과가 있어야 상세 리포트를 볼 수 있습니다." actions={<ActionButton onClick={() => go('projects')}>프로젝트 목록</ActionButton>}>
        <StatePanel type="empty" title="분석된 프로젝트가 없습니다" desc="실제 project inventory API에서 스킬과 경험 태그가 내려오면 상세 분석 화면을 표시합니다. 현재는 가짜 분석 결과를 만들지 않습니다." action="프로젝트 설정 확인" />
      </Shell>;
    }
