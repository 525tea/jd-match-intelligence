...
export function JobFlowScreens({ t, go, screen }) {
      ...
      return <Shell title="프로젝트" sub="프로젝트 분석 결과가 연결되면 추출 스킬, 경험 태그, 매칭 공고가 이 화면에 표시됩니다." actions={<ActionButton onClick={() => go('jobs')}>공고 먼저 보기</ActionButton>}>
        <div style={{ display: 'grid', gridTemplateColumns: narrow ? '1fr' : '1.05fr 0.95fr', gap: 18, alignItems: 'stretch' }}>
          <section style={{ ...tile, padding: 28 }}>
            ...
            <p style={{ color: muted, fontSize: 15, lineHeight: 1.7, margin: '14px 0 0' }}>현재 프론트는 실제 project inventory API 응답만 프로젝트 카드로 보여줍니다. 로그인 화면에서 분석 프로젝트 ID를 입력하거나 로컬 환경변수에 기본 프로젝트 ID를 설정하면 이 화면이 채워집니다.</p>
            <div style={{ marginTop: 18, display: 'grid', gap: 10 }}>
              <StatePanel type={projectApiMissing ? 'unauthorized' : 'empty'} title={projectApiMissing ? '프로젝트 ID 설정이 필요합니다' : '프로젝트 분석 결과 대기'} desc={projectApiMissing ? '백엔드에 없는 프로젝트를 임의로 호출하지 않고, 프로젝트 ID가 설정될 때까지 분석 화면을 비워둡니다.' : '프로젝트 분석 API가 성공하면 스킬과 경험 태그가 카드로 표시됩니다.'} action="로그인 / 데모 설정 확인" />
              <StatePanel type="forbidden" title="새 레포 분석 API는 아직 연결 전입니다" desc="프로젝트 생성·재분석 endpoint가 생기기 전까지는 화면에서 임의 분석 성공처럼 이동하지 않습니다." secondary="공개 공고 탐색" />
            </div>
          </section>
  ...
  const projectNewScreen = () => <Shell title="새 레포 분석" sub="현재 프론트는 기존 분석 결과 조회 API에 연결되어 있습니다. 새 레포 등록과 재분석 요청은 백엔드 endpoint가 추가된 뒤 연결합니다." actions={<ActionButton onClick={() => go('projects')}>프로젝트 목록</ActionButton>}>
    <div style={{ display: 'grid', gridTemplateColumns: narrow ? '1fr' : '1.05fr 0.95fr', gap: 18, alignItems: 'start' }}>
      <section style={{ ...tile, padding: 28 }}>
        ...
        <p style={{ color: muted, fontSize: 15, lineHeight: 1.72, margin: '14px 0 0' }}>프로젝트 스킬, 경험 태그, JD 매칭, 갭 분석, 추천 공고는 실제 API 응답으로 표시됩니다. 새 레포를 등록하거나 기존 레포를 재분석하는 API는 아직 백엔드 범위에 없어서 이 화면에서 성공 처리하지 않습니다.</p>
  ...
  const projectAnalysisScreen = () => {
    if (!JF.__userProjectId || !JF.projectList.length) {
      return <Shell title="프로젝트 분석" sub="프로젝트 분석 결과가 있어야 상세 리포트를 볼 수 있습니다." actions={<ActionButton onClick={() => go('projects')}>프로젝트 목록</ActionButton>}>
        <StatePanel type="empty" title="분석된 프로젝트가 없습니다" desc="실제 project inventory API에서 스킬과 경험 태그가 내려오면 상세 분석 화면을 표시합니다. 현재는 임의 분석 결과를 만들지 않습니다." action="프로젝트 설정 확인" />
      </Shell>;
    }
  ...
  const recommendationsScreen = () => {
    ...
    const recommendationState = recommendationApiMissing
      ? { type: 'empty', title: '프로젝트 ID가 필요합니다', desc: '추천 API는 분석된 프로젝트 ID 기준으로 계산됩니다. 데모 프로젝트 ID를 설정하거나 프로젝트 분석 결과를 먼저 연결하세요.' }
      : recommendationApiUnavailable
        ? { type: 'error', title: '추천 API를 불러오지 못했습니다', desc: '백엔드 추천 API 응답을 받지 못했습니다. 토큰, 프로젝트 ID, gateway 상태를 확인한 뒤 다시 시도하세요.' }
