...
export function JobFlowScreens({ t, go, screen }) {
  ...
  const demoStatusScreen = () => {
    ...
    const services = [['api-gateway', 'UP', 8081, 42, 99.98], ['backend', apiHealthOk ? 'UP' : 'CHECK', 8080, 42, 99.98], ['mysql-primary', 'UP', 3306, 8, 99.99], ['redis-cache', 'UP', 6379, 1, 100], ['elasticsearch', 'UP', 9200, 23, 99.95], ['prometheus', 'UP', 9090, 12, 99.97], ['grafana', 'UP', 3001, 18, 99.96], ['zipkin', 'UP', 9411, 31, 99.9]];
    ...
    return <Shell title="Demo Status" sub="포트폴리오 시연용 운영 콘솔 — 서비스 헬스, 파이프라인, smoke/CI, 품질 지표를 실시간 대시보드로 보여줍니다." actions={<div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}><ActionButton onClick={() => go('home')}>사용자 홈</ActionButton><ActionButton primary onClick={() => go('project-analysis')}>핵심 플로우 보기</ActionButton></div>}>
      <div style={{ background: '#0a0d12', borderRadius: 24, padding: narrow ? 16 : 24, boxShadow: '0 24px 60px rgba(10,13,18,0.4)', backgroundImage: 'linear-gradient(#11151c 1px, transparent 1px), linear-gradient(90deg, #11151c 1px, transparent 1px)', backgroundSize: '34px 34px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: narrow ? '1fr 1fr' : 'repeat(4, 1fr)', gap: 12 }}>
          <KPI label="REAL JOBS INDEXED" value={String(JF.market?.totalCount || JF.listings?.length || 0)} sub="gateway API / local fixture" seed={1} />
        ...
        <div style={{ display: 'grid', gridTemplateColumns: narrow ? '1fr' : '1.05fr 0.95fr', gap: 14, marginTop: 14 }}>
          <section style={{ background: '#0e1116', border: '1px solid #1c2230', borderRadius: 16, padding: 18 }}>
            ...
            <div style={{ display: 'grid', gap: 0, marginTop: 8 }}>{services.map(([name, st, port, lat, up]) => <div key={name} style={{ display: 'grid', gridTemplateColumns: '16px 1fr 58px 64px', gap: 10, alignItems: 'center', padding: '11px 0', borderTop: '1px solid #161b24' }}><Dot ok={st==='UP'} /><span style={{ fontFamily: mono, fontSize: 13, color: '#dfe5ec', fontWeight: 600 }}>{name}<span style={{ color: '#4b5666' }}>:{port}</span></span><span style={{ fontFamily: mono, fontSize: 12, color: '#8b95a4', textAlign: 'right' }}>{lat}ms</span><span style={{ fontFamily: mono, fontSize: 12, color: st === 'UP' ? green : coral, textAlign: 'right', fontWeight: 700 }}>{st === 'UP' ? `${up}%` : 'check'}</span></div>)}</div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: narrow ? '1fr' : '1.25fr 0.75fr', gap: 14, marginTop: 14 }}>
          ...
          <section style={{ display: 'grid', gap: 12, gridAutoRows: 'min-content' }}>
            <div style={{ background: '#0e1116', border: '1px solid #1c2230', borderRadius: 16, padding: 18 }}><div style={{ fontFamily: mono, color: '#5f6b7d', fontSize: 11, fontWeight: 800, letterSpacing: 1 }}>FRONTEND API STATE</div><div style={{ display: 'grid', gap: 7, marginTop: 10 }}>{(apiStatusEntries.length ? apiStatusEntries : [['preview', 'mock']]).map(([name, status]) => <div key={name} style={{ display: 'grid', gridTemplateColumns: '1fr 58px', gap: 10, alignItems: 'center', fontFamily: mono, fontSize: 11.5 }}><span style={{ color: '#aeb7c4' }}>{name}</span><b style={{ color: status === 'ok' ? green : coral, textAlign: 'right' }}>{status}</b></div>)}</div></div>
