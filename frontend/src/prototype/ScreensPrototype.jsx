...
export function JobFlowScreens({ t, go, screen }) {
    ...
    <div style={{ display: 'grid', gridTemplateColumns: narrow ? '1fr' : '1.05fr 0.95fr', gap: 18, alignItems: 'start' }}>
      ...
      <aside style={{ display: 'grid', gap: 14 }}>
        <section style={{ ...tile, background: ink, color: '#fff' }}>
          ...
          <div style={{ fontSize: 22, fontWeight: 900, letterSpacing: -0.5, lineHeight: 1.28 }}>새 레포를 연결하면 분석부터 추천까지 순서대로 진행됩니다.</div>
          <div style={{ display: 'grid', gap: 10, marginTop: 18 }}>
            {[
              ['레포 등록', 'GitHub 레포 주소와 기준 브랜치를 선택합니다.'],
  ...
  const demoStatusScreen = () => {
    ...
    const apiHealthLabel = apiStatusEntries.length && apiUnavailableCount === 0 ? '정상 연결' : apiStatusEntries.length ? '일부 점검 필요' : '미리보기';
    const apiHealthOk = apiStatusEntries.length > 0 && apiOkCount > 0;
    const services = [['API 라우팅', apiHealthOk ? '정상' : '확인 필요', 8081], ['애플리케이션 서버', apiHealthOk ? '정상' : '확인 필요', 8080], ['데이터베이스', '설정됨', 3306], ['캐시', '설정됨', 6379], ['검색 인덱스', '설정됨', 9200], ['메트릭 수집', '설정됨', 9090], ['대시보드', '설정됨', 3001], ['트레이싱', '설정됨', 9411]];
    ...
    return <Shell title="System Status" sub="서비스 헬스, 데이터 파이프라인, API 연결 상태, 관측 지표를 한 화면에서 확인합니다." actions={<div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}><ActionButton onClick={() => go('home')}>사용자 홈</ActionButton><ActionButton primary onClick={() => go('project-analysis')}>핵심 플로우 보기</ActionButton></div>}>
      <div style={{ background: '#0a0d12', borderRadius: 24, padding: narrow ? 16 : 24, boxShadow: '0 24px 60px rgba(10,13,18,0.4)', backgroundImage: 'linear-gradient(#11151c 1px, transparent 1px), linear-gradient(90deg, #11151c 1px, transparent 1px)', backgroundSize: '34px 34px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: narrow ? '1fr 1fr' : 'repeat(4, 1fr)', gap: 12 }}>
          ...
          <KPI label="분석 프로젝트" value={String(JF.projectList?.length || 0)} sub="현재 계정 기준" seed={3} />
        ...
        <div style={{ display: 'grid', gridTemplateColumns: narrow ? '1fr' : '1.05fr 0.95fr', gap: 14, marginTop: 14 }}>
          ...
          <section style={{ background: '#0e1116', border: '1px solid #1c2230', borderRadius: 16, padding: 18 }}>
            ...
            <div style={{ display: 'grid', gap: 9, marginTop: 12 }}>{[['분석', '레포 분석 → 스킬·경험 태그 추출', green], ['색인', '공고 수집 → 정규화 → 검색 인덱스 반영', green], ['매칭', 'JD 매칭 → 갭 분석 → 추천 피드', green], ['알림', '저장 공고 → 배치 발송 → 재시도 관리', green]].map(([tag, txt], k) => <div key={tag} style={{ display: 'flex', alignItems: 'center', gap: 11, padding: '11px 13px', background: '#11151c', border: '1px solid #1c2230', borderRadius: 12 }}><span style={{ fontFamily: mono, fontSize: 10.5, fontWeight: 800, color: green, background: 'rgba(185,236,42,0.1)', border: '1px solid rgba(185,236,42,0.25)', borderRadius: 7, padding: '3px 7px', minWidth: 52, textAlign: 'center' }}>{tag}</span><span style={{ fontFamily: mono, fontSize: 12.5, color: '#cdd4dd', fontWeight: 600, lineHeight: 1.4 }}>{txt}</span><span style={{ marginLeft: 'auto', color: green, fontSize: 13 }}>✓</span></div>)}</div>
          </section>
        </div>
        {/* smoke + quality */}
