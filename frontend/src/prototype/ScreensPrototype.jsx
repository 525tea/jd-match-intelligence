...
export function JobFlowScreens({ t, go, screen }) {
  ...
  const [previewProject, setPreviewProject] = React.useState('commerce-api');
  ...
  const JobThumbnail = ({ job, score, compact, locked }) => {
    ...
    return <div className="jf-jobcard" onClick={() => go('detail', { jobId: job.jobId || job.id, company: job.companyKo })} style={{ cursor: 'pointer', ...tile, border: '1px solid ' + (isV3 ? '#edf0f3' : '#edf0f3'), boxShadow: isV3 ? '0 1px 2px rgba(20,21,26,0.025), 0 8px 20px rgba(20,21,26,0.035)' : '0 1px 2px rgba(20,21,26,0.03), 0 7px 18px rgba(20,21,26,0.045)', padding: isV3 ? 17 : 18, display: 'flex', flexDirection: 'column', minHeight: compact ? 186 : 208 }}>
      ...
      <div style={{ marginTop: 'auto', paddingTop: 15, display: 'flex', alignItems: 'center', gap: 10, color: faint, fontSize: 11.5, fontWeight: 650 }}>
        ...
        <button onClick={async (e) => { e.stopPropagation(); try { await window.__jobflowApi?.saveJobByCompany?.(job.companyKo); } catch (_) {} }} aria-label="공고 저장" style={{ width: 28, height: 28, borderRadius: 10, border: '1px solid ' + line, background: '#fff', color: muted, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}><BookmarkIcon /></button>
      </div>
    </div>;
  };
          ...
          <section>
            ...
            <div style={{ ...tile, padding: narrow ? 16 : 18, marginBottom: 14, display: 'grid', gap: 14 }}>
              <div style={{ display: 'grid', gridTemplateColumns: narrow ? '1fr auto' : '1fr auto', gap: 8 }}>
                ...
                <button onClick={() => narrow ? setFilterOpen(!filterOpen) : window.__jobflowApi?.searchJobs?.(query)} style={{ font: 'inherit', cursor: 'pointer', border: 'none', background: ink, color: '#fff', borderRadius: 14, padding: '0 20px', fontWeight: 800 }}>{narrow ? (filterOpen ? '닫기' : '필터') : '검색'}</button>
              </div>
  ...
  const projectNewScreen = () => <Shell title="새 레포 분석" sub="GitHub 레포 URL을 등록하고 분석이 끝나면 프로젝트 상세와 추천 공고로 이어집니다." actions={<ActionButton onClick={() => go('projects')}>프로젝트 목록</ActionButton>}>
    <div style={{ display: 'grid', gridTemplateColumns: narrow ? '1fr' : '1fr 0.86fr', gap: 18, alignItems: 'start' }}>
      <section style={{ ...tile, padding: 26 }}>
        ...
        <H>분석할 레포지토리</H>
        <Field label="프로젝트 이름" value="commerce-api" />
        <Field label="GitHub 레포 URL" value="https://github.com/example-org/commerce-api" />
  ...
  const applicationsScreen = () => {
    ...
    return <Shell title="지원 현황" sub="외부에서 지원한 공고를 단계별로 추적하고, 상태를 바로 변경합니다." actions={<ActionButton primary onClick={() => setAddingApplication(true)}>지원 기록 추가</ActionButton>}>
      ...
      <div style={{ ...tile, padding: 0, overflow: 'hidden' }}>
        ...
        {JF.applications.map((a, i) => { const c = stageColor[a.status] || stageColor.APPLIED; return <div key={a.id || a.company} style={{ display: 'grid', gridTemplateColumns: narrow ? '1fr 116px' : '1fr 120px 150px 128px', gap: 14, padding: '15px 22px', borderTop: i ? '1px solid ' + line : 'none', alignItems: 'center' }}><div onClick={() => go('detail', { jobId: a.jobId, company: a.company })} style={{ display: 'flex', alignItems: 'center', gap: 11, minWidth: 0, cursor: 'pointer' }}><div style={{ width: 36, height: 36, borderRadius: 11, background: ink, color: green, display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 900, fontSize: 13, flexShrink: 0 }}>{a.company.slice(0, 2)}</div><div style={{ minWidth: 0 }}><b style={{ fontSize: 14.5 }}>{a.company}</b><div style={{ color: muted, fontSize: 12.5, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{a.title}</div></div></div>{!narrow && <><span style={{ color: muted, fontSize: 13, ...num }}>{a.appliedAt ? String(a.appliedAt).slice(0, 10).replaceAll('-', '.') : `2026.06.${String(9 + i).padStart(2, '0')}`}</span><select defaultValue={a.status} onChange={(e) => window.__jobflowApi?.updateApplicationStatus?.(a.id, e.target.value).catch(() => {})} style={{ font: 'inherit', border: '1px solid ' + line, background: '#fff', borderRadius: 12, padding: '8px 10px', fontSize: 12.5, fontWeight: 800 }}><option value="APPLIED">지원 완료</option><option value="DOCUMENT_PASSED">서류 합격</option><option value="CODING_TEST">코딩테스트</option><option value="INTERVIEW">면접 진행</option><option value="OFFER">오퍼</option><option value="REJECTED">불합격</option><option value="WITHDRAWN">지원 취소</option></select></>}<span style={{ justifySelf: 'end', fontSize: 12, fontWeight: 800, color: c.fg, background: c.bg, border: '1px solid ' + c.bd, borderRadius: 14, padding: '5px 11px', whiteSpace: 'nowrap' }}>{statusLabel[a.status] || a.status}</span></div>; })}
      </div>
      {addingApplication && <Modal title="지원 기록 추가" sub="외부 채용 사이트에서 지원한 공고도 JobFlow에서 단계별로 추적합니다." onClose={() => setAddingApplication(false)}><Field label="회사명" value="코어페이" /><Field label="공고명" value="코어페이 결제 플랫폼 백엔드 엔지니어 신입 채용" /><div style={{ display: 'grid', gridTemplateColumns: narrow ? '1fr' : '1fr 1fr', gap: 10 }}><Field label="지원일" value="2026.06.17" /><label><div style={{ fontSize: 12, color: muted, fontWeight: 850, marginBottom: 6 }}>현재 단계</div><select defaultValue="APPLIED" style={{ width: '100%', font: 'inherit', border: '1px solid ' + line, borderRadius: 13, padding: '12px 13px', background: '#fff' }}><option value="APPLIED">지원 완료</option><option value="DOCUMENT_PASSED">서류 합격</option><option value="CODING_TEST">코딩테스트</option><option value="INTERVIEW">면접 진행</option><option value="OFFER">오퍼</option></select></label></div><Field label="메모" value="지원 후 포트폴리오 링크와 commerce-api 설명을 보강할 것" textarea /><div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', marginTop: 12 }}><ActionButton onClick={() => setAddingApplication(false)}>취소</ActionButton><ActionButton primary onClick={async () => { try { await window.__jobflowApi?.createApplicationByCompany?.('코어페이'); } catch (_) {} setAddingApplication(false); }}>기록 저장</ActionButton></div></Modal>}
    </Shell>;
  };
