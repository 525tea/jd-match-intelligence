...
export function JobFlowScreens({ t, go, screen }) {
  ...
  const demoStatusScreen = () => {
    ...
    return <Shell title="시스템 상태" sub="서비스 헬스, 데이터 파이프라인, API 연결 상태, 관측 지표를 한 화면에서 확인합니다." actions={<div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}><ActionButton onClick={() => go('home')}>사용자 홈</ActionButton><ActionButton primary onClick={() => go('project-analysis')}>핵심 플로우 보기</ActionButton></div>}>
