...
export function JobFlowScreens({ t, go, screen }) {
  ...
  const [trendSkill, setTrendSkill] = React.useState(() => window.JF?.trends?.[0]?.name || '');
  ...
  const selectedTrend = JF.trends.find((item) => item.name === trendSkill) || JF.trends[0] || { name: '트렌드 없음', rate: 0, growth: 0, owned: false, insight: '트렌드 API 결과가 없습니다.' };
  ...
  const JobThumbnail = ({ job, score, compact, locked, scoreLabel = '레포 매칭' }) => {
  ...
  const jobsScreen = () => {
    ...
    return (
      <Shell title="공고" sub={isV3 ? "직무·경력·스택·지역 조건으로 먼저 좁힌 뒤 검색 점수와 상세 조건으로 확인합니다." : (login ? "검색어를 먼저 입력하고, 직무·경력·스킬·경험 태그로 좁힌 뒤 프로젝트 매칭 화면으로 이어갑니다." : "로그인 없이도 전체 공고와 필터를 먼저 탐색할 수 있어요. 레포를 연결하면 추천 화면에서 매칭순으로 볼 수 있습니다.")}>
        <div style={{ display: 'grid', gridTemplateColumns: narrow ? '1fr' : '280px 1fr', gap: 22, alignItems: 'start' }}>
          ...
          <section>
            ...
            {JF.__apiStatus?.jobs === 'unavailable' && <div style={{ marginBottom: 14 }}><StatePanel type="error" title="실시간 공고 API를 불러오지 못했습니다" desc="백엔드 또는 게이트웨이 연결을 확인한 뒤 다시 시도해주세요." action="다시 시도" /></div>}
            {isV3 && <div style={{ display: 'grid', gridTemplateColumns: narrow ? '1fr 1fr' : 'repeat(3, 1fr)', gap: 10, marginBottom: 14 }}>{[['전체 공고', JF.market.totalCount.toLocaleString()], ['현재 목록', JF.listings.length.toLocaleString()], ['마감 정보 있음', JF.listings.filter((job) => job.deadline && job.deadline !== '상시').length.toLocaleString()]].map(([l, v]) => <div key={l} style={{ background: '#fff', border: '1px solid ' + line, borderRadius: 16, padding: '12px 14px' }}><b style={{ fontSize: 20, ...num }}>{v}</b><div style={{ fontSize: 12, color: muted, fontWeight: 800, marginTop: 2 }}>{l}</div></div>)}</div>}
            <div style={{ background: 'linear-gradient(135deg, #ffffff 0%, #fbfcfd 62%, #eef8cf 180%)', border: '1px solid ' + line, borderRadius: 26, padding: narrow ? 18 : 22, marginBottom: 16, display: 'grid', gap: 16, boxShadow: '0 14px 38px rgba(20,21,26,0.07)' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: narrow ? '1fr auto' : '1fr auto', gap: 8 }}>
	                <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder={isV3 ? "검색어는 선택 사항이에요. 먼저 필터로 좁혀보세요." : "예: 백엔드 Spring, Kubernetes 플랫폼, C++ 개발자"} style={{ width: '100%', minWidth: 0, border: '1px solid #dfe3ea', background: '#fff', color: ink, borderRadius: 16, padding: '15px 16px', font: 'inherit', fontWeight: isV3 ? 600 : 750, outline: 'none', boxShadow: '0 1px 2px rgba(20,21,26,0.04)' }} />
                ...
                }} style={{ font: 'inherit', cursor: 'pointer', border: 'none', background: ink, color: '#fff', borderRadius: 16, padding: '0 22px', fontWeight: 900 }}>{narrow ? (filterOpen ? '닫기' : '필터') : '검색'}</button>
              </div>
  ...
  const projectsScreen = () => {
    ...
    const hasAnalyzedProject = Boolean(JF.__userProjectId && JF.projectList.length);
      ...
      return <Shell title="프로젝트" sub="프로젝트 분석 결과가 연결되면 추출 스킬, 경험 태그, 매칭 공고가 이 화면에 표시됩니다." actions={<ActionButton onClick={() => go('jobs')}>공고 먼저 보기</ActionButton>}>
        <div style={{ display: 'grid', gridTemplateColumns: narrow ? '1fr' : '1.05fr 0.95fr', gap: 18, alignItems: 'stretch' }}>
          <section style={{ ...tile, padding: 28 }}>
            <SecTitle>PROJECT INVENTORY</SecTitle>
            <div style={{ fontSize: narrow ? 28 : 34, fontWeight: 950, letterSpacing: -1, lineHeight: 1.16 }}>아직 연결된 프로젝트 분석 결과가 없습니다.</div>
            <p style={{ color: muted, fontSize: 15, lineHeight: 1.7, margin: '14px 0 0' }}>로그인 화면에서 분석 프로젝트 ID를 입력하면 스킬, 경험 태그, 매칭 공고가 이 화면에 채워집니다.</p>
            <div style={{ marginTop: 18, display: 'grid', gap: 10 }}>
              <StatePanel type={projectApiMissing ? 'unauthorized' : 'empty'} title={projectApiMissing ? '프로젝트 ID 설정이 필요합니다' : '프로젝트 분석 결과 대기'} desc={projectApiMissing ? '분석된 프로젝트 ID를 입력하면 스킬, 경험 태그, 매칭 공고를 불러옵니다.' : '프로젝트 분석이 완료되면 스킬과 경험 태그가 카드로 표시됩니다.'} action="로그인 / 프로젝트 설정 확인" />
              <StatePanel type="forbidden" title="새 레포 분석 준비 중" desc="지금은 이미 분석된 프로젝트 결과를 기준으로 추천과 갭 분석을 확인할 수 있습니다." secondary="공개 공고 탐색" />
            </div>
          </section>
          <aside style={{ ...tile, background: ink, color: '#fff', display: 'flex', flexDirection: 'column' }}>
            <SecTitle color={green}>CONNECTED FLOW</SecTitle>
    ...
    return <Shell title="프로젝트" sub="전체 프로젝트와 추출 스킬 목록입니다. 프로젝트를 누르면 상세 분석으로 들어가고, 매칭 공고는 우측에서 미리 봅니다." actions={<button onClick={() => go('project-new')} style={{ font: 'inherit', cursor: 'pointer', border: 'none', background: ink, color: '#fff', borderRadius: 22, padding: '12px 18px', fontWeight: 900 }}>+ 새 레포 분석</button>}>
      {projectApiUnavailable && <div style={{ display: 'grid', gap: 14, marginBottom: 18 }}><StatePanel type="error" title="프로젝트 API 일부를 불러오지 못했습니다" desc="프로젝트 스킬, 경험 태그, 매칭 공고 중 일부 요청이 실패했습니다. 현재 화면은 성공한 API 응답만 기준으로 표시합니다." action="새로고침 후 재시도" /></div>}
  ...
  const projectNewScreen = () => <Shell title="새 레포 분석" sub="지금은 분석된 프로젝트 결과를 기준으로 추천과 갭 분석을 확인합니다. 새 레포 등록은 다음 단계에서 연결됩니다." actions={<ActionButton onClick={() => go('projects')}>프로젝트 목록</ActionButton>}>
    <div style={{ display: 'grid', gridTemplateColumns: narrow ? '1fr' : '1.05fr 0.95fr', gap: 18, alignItems: 'start' }}>
      <section style={{ ...tile, padding: 28 }}>
        <SecTitle>AVAILABLE NOW</SecTitle>
        <div style={{ fontSize: narrow ? 28 : 34, fontWeight: 950, letterSpacing: -1, lineHeight: 1.15 }}>지금은 분석된 프로젝트를 기준으로 추천과 갭 분석을 확인합니다.</div>
        <p style={{ color: muted, fontSize: 15, lineHeight: 1.72, margin: '14px 0 0' }}>프로젝트를 연결하면 스킬, 경험 태그, JD 매칭, 갭 분석, 추천 공고가 한 흐름으로 이어집니다. 분석 결과가 있는 프로젝트를 선택하면 바로 매칭 화면으로 이동할 수 있어요.</p>
      ...
      <aside style={{ display: 'grid', gap: 14 }}>
        <section style={{ ...tile, background: ink, color: '#fff' }}>
          <SecTitle color={green}>NEXT BACKEND API</SecTitle>
          <div style={{ fontSize: 22, fontWeight: 900, letterSpacing: -0.5, lineHeight: 1.28 }}>새 레포 분석 버튼을 살리려면 아래 API가 필요합니다.</div>
          <div style={{ display: 'grid', gap: 10, marginTop: 18 }}>
            {[
              ['POST /projects', 'GitHub repository URL과 기준 브랜치를 저장합니다.'],
              ['POST /projects/{id}/analyses', '정적 분석 작업을 시작하고 analysis id를 반환합니다.'],
              ['GET /projects/{id}/analyses/latest', '분석 진행 상태와 최신 결과를 조회합니다.'],
        ...
        <section style={tile}>
          <SecTitle>_CURRENT PROJECT</SecTitle>
          <div style={{ display: 'grid', gap: 10 }}>
            {[
              ['프로젝트', currentProjectName],
              ['레포', currentProjectRepo],
  ...
  const projectAnalysisScreen = () => {
    if (!JF.__userProjectId || !JF.projectList.length) {
    ...
    return <Shell title={p.name}
      ...
      <div style={{ display: 'grid', gap: 18 }}>
        {analysisApiUnavailable && <div style={{ display: 'grid', gridTemplateColumns: narrow ? '1fr' : 'repeat(3, 1fr)', gap: 12 }}>
          <StatePanel type="error" title="프로젝트 분석 API 일부를 불러오지 못했습니다" desc="스킬, 경험 태그, 매칭 공고 중 일부 요청이 실패했습니다. 성공한 응답만 화면에 표시합니다." action="새로고침 후 재시도" />
        </div>}
  ...
  const trendsScreen = () => {
    ...
    return <Shell title="트렌드" sub="이번 달 공고에서 많이 요구된 스킬을 집계하고, 내 프로젝트에 없는 스킬은 바로 공고·갭 분석으로 이어갑니다." actions={<select style={{ font: 'inherit', border: '1px solid ' + line, background: '#fff', borderRadius: 12, padding: '11px 14px', fontWeight: 900 }}><option>전체 공고 기준</option><option>백엔드 공고 기준</option><option>프론트엔드 공고 기준</option><option>AI 공고 기준</option></select>}>
      <div style={{ display: 'grid', gap: 18 }}>
        <div style={{ display: 'grid', gridTemplateColumns: narrow ? '1fr' : '1.2fr 1fr', gap: 0, background: ink, color: '#fff', borderRadius: 24, overflow: 'hidden', boxShadow: '0 14px 30px rgba(20,21,26,0.16)' }}>
          <div style={{ padding: narrow ? 22 : 30 }}>
            <SecTitle color={green}>MONTHLY SKILL TREND</SecTitle>
            <div style={{ fontSize: narrow ? 26 : 34, fontWeight: 950, lineHeight: 1.2, letterSpacing: -1 }}>이번 달 공고에서 <span style={{ color: green }}>{topTrend.name}</span>가 가장 많이 보입니다.</div>
            <p style={{ color: 'rgba(255,255,255,0.68)', lineHeight: 1.72, margin: '14px 0 0' }}>{topTrend.insight || '트렌드 API 결과를 기반으로 보유 스킬과 미보유 스킬의 차이를 보여줍니다.'}</p>
  ...
  const demoStatusScreen = () => {
    ...
    return <Shell title="System Status" sub="서비스 헬스, 데이터 파이프라인, API 연결 상태, 관측 지표를 한 화면에서 확인합니다." actions={<div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}><ActionButton onClick={() => go('home')}>사용자 홈</ActionButton><ActionButton primary onClick={() => go('project-analysis')}>핵심 플로우 보기</ActionButton></div>}>
      <div style={{ background: '#0a0d12', borderRadius: 24, padding: narrow ? 16 : 24, boxShadow: '0 24px 60px rgba(10,13,18,0.4)', backgroundImage: 'linear-gradient(#11151c 1px, transparent 1px), linear-gradient(90deg, #11151c 1px, transparent 1px)', backgroundSize: '34px 34px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14, flexWrap: 'wrap', padding: '4px 4px 18px', borderBottom: '1px solid #1c2230', marginBottom: 18 }}>
          ...
          <span style={{ fontFamily: mono, color: '#5f6b7d', fontSize: 12.5 }}>env=<span style={{ color: '#aeb7c4' }}>local</span></span>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: narrow ? '1fr' : '1.05fr 0.95fr', gap: 14, marginTop: 14 }}>
          <section style={{ background: '#0e1116', border: '1px solid #1c2230', borderRadius: 16, padding: 18 }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}><span style={{ fontFamily: mono, color: '#5f6b7d', fontSize: 11, fontWeight: 800, letterSpacing: 1 }}>SYSTEM HEALTH</span><span style={{ fontFamily: mono, color: apiHealthOk ? green : coral, fontSize: 11, fontWeight: 800 }}>{apiStatusEntries.length ? `${apiOkCount}/${apiStatusEntries.length} API OK` : 'NOT CHECKED'}</span></div>
            <div style={{ display: 'grid', gap: 0, marginTop: 8 }}>{services.map(([name, st, port]) => <div key={name} style={{ display: 'grid', gridTemplateColumns: '16px 1fr 88px', gap: 10, alignItems: 'center', padding: '11px 0', borderTop: '1px solid #161b24' }}><Dot ok={st==='UP'} /><span style={{ fontFamily: mono, fontSize: 13, color: '#dfe5ec', fontWeight: 600 }}>{name}<span style={{ color: '#4b5666' }}>:{port}</span></span><span style={{ fontFamily: mono, fontSize: 12, color: st === 'UP' ? green : '#8b95a4', textAlign: 'right', fontWeight: 700 }}>{st}</span></div>)}</div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: narrow ? '1fr' : '1.25fr 0.75fr', gap: 14, marginTop: 14 }}>
          ...
          <section style={{ display: 'grid', gap: 12, gridAutoRows: 'min-content' }}>
            <div style={{ background: '#0e1116', border: '1px solid #1c2230', borderRadius: 16, padding: 18 }}><div style={{ fontFamily: mono, color: '#5f6b7d', fontSize: 11, fontWeight: 800, letterSpacing: 1 }}>FRONTEND API STATE</div><div style={{ display: 'grid', gap: 7, marginTop: 10 }}>{(apiStatusEntries.length ? apiStatusEntries : [['api', 'unavailable']]).map(([name, status]) => <div key={name} style={{ display: 'grid', gridTemplateColumns: '1fr 58px', gap: 10, alignItems: 'center', fontFamily: mono, fontSize: 11.5 }}><span style={{ color: '#aeb7c4' }}>{name}</span><b style={{ color: status === 'ok' ? green : coral, textAlign: 'right' }}>{status}</b></div>)}</div></div>
            <div style={{ background: '#0e1116', border: '1px solid #1c2230', borderRadius: 16, padding: 18 }}><div style={{ fontFamily: mono, color: '#5f6b7d', fontSize: 11, fontWeight: 800, letterSpacing: 1 }}>PROJECT SKILLS</div><div style={{ display: 'flex', alignItems: 'baseline', gap: 6, marginTop: 8 }}><b style={{ fontFamily: mono, fontSize: 30, color: green, letterSpacing: -1 }}>{JF.skills?.length || 0}</b><span style={{ marginLeft: 'auto', fontFamily: mono, fontSize: 11, color: apiHealthOk ? green : coral, background: 'rgba(185,236,42,0.1)', border: '1px solid rgba(185,236,42,0.25)', borderRadius: 7, padding: '3px 8px', fontWeight: 800 }}>{apiHealthOk ? 'CONNECTED' : 'CHECK'}</span></div><div style={{ color: '#5f6b7d', fontSize: 11.5, marginTop: 6, fontFamily: mono }}>project skill inventory</div></div>
            <div style={{ background: '#0e1116', border: '1px solid #1c2230', borderRadius: 16, padding: 18 }}><div style={{ fontFamily: mono, color: '#5f6b7d', fontSize: 11, fontWeight: 800, letterSpacing: 1 }}>TREND SKILLS</div><b style={{ display: 'block', fontFamily: mono, fontSize: 30, color: '#eef2f7', letterSpacing: -1, marginTop: 8 }}>{JF.trends?.length || 0}</b><div style={{ color: '#5f6b7d', fontSize: 11.5, marginTop: 6, fontFamily: mono }}>trend API rows</div></div>
  ...
  const applicationsScreen = () => {
    ...
    return <Shell title="지원 현황" sub="외부에서 지원한 공고를 단계별로 추적하고, 상태를 바로 변경합니다." actions={<ActionButton primary onClick={() => setAddingApplication(true)}>지원 기록 추가</ActionButton>}>
      ...
      {JF.__apiStatus?.applications === 'unavailable' && <div style={{ marginBottom: 18 }}><StatePanel type="error" title="지원 현황 API를 불러오지 못했습니다" desc="백엔드 또는 게이트웨이 연결을 확인한 뒤 다시 시도해주세요." action="다시 시도" /></div>}
