      ...
      <main style={{ maxWidth: 1520, margin: '0 auto', padding: narrow ? '28px 18px 54px' : '36px 48px 64px' }}>
        <section style={{ display: 'grid', gridTemplateColumns: narrow ? '1fr' : (v3 ? '0.92fr 1.08fr' : '1.05fr 0.82fr'), gap: 18, alignItems: 'stretch', marginBottom: 34 }}>
          <div style={{ background: green, borderRadius: 28, padding: narrow ? 26 : (v3 ? 28 : 34), minHeight: v3 ? 220 : 254, display: 'flex', flexDirection: 'column' }}>
            ...
            <h1 style={{ fontSize: narrow ? 34 : 50, lineHeight: 1.04, letterSpacing: -2, margin: '38px 0 14px', maxWidth: 760 }}>대량 공고 중 내 프로젝트와<br />맞는 공고만 <span style={{ background: ink, color: green, borderRadius: 12, padding: '0 14px', display: 'inline-block' }}>골라서.</span></h1>
            <p style={{ color: 'rgba(20,21,26,0.65)', fontSize: 16, lineHeight: 1.65, maxWidth: 620 }}>지금 <b>{JF.market.totalCount.toLocaleString()}개</b> 공고를 모아 기술 스택·경험 태그·경력으로 좁힌 뒤, 프로젝트 기준 매칭률로 다시 정렬해드려요.</p>
