...
export function toPrototypeJob(job, index = 0) {
  ...
  return {
    ...
    source,
    externalId,
    canonicalFingerprint: rawJob.canonicalFingerprint || rawJob.canonical_fingerprint || job.canonicalFingerprint || job.canonical_fingerprint,
    status: rawJob.status || job.status,
    sourceId: externalId,
    applyUrl: rawJob.applyUrl || rawJob.apply_url || job.applyUrl || job.apply_url || '',
    originalUrl: rawJob.applyUrl || rawJob.apply_url || job.applyUrl || job.apply_url || canonicalApplyUrl(source, externalId, rawJob.originalUrl || rawJob.original_url || rawJob.url || job.originalUrl || job.original_url || job.url),
