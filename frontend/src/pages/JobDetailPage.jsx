  ...
  return <main className="page">
    ...
    <div className="detail-layout">
      ...
      <aside className="grid">
        <section className="tile side-card"><p className="eyebrow">MATCH REPORT</p><h2 style={{ margin: 0 }}>commerce-api 기준 {score}%</h2><ProgressBar value={score} /><div className="filter-group"><h4>보유 스킬</h4><div className="skill-row">{matched.map((s) => <Badge key={s} tone="skill">✓ {s}</Badge>)}</div></div><div className="filter-group"><h4>미보유 스킬</h4><div className="skill-row">{missing.map((s) => <Badge key={s} tone="muted">{s}</Badge>)}</div></div><Button variant="dark" onClick={() => go('projects')}>다른 프로젝트로 다시 매칭</Button></section>
