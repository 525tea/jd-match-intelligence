...
export function RecommendationsPage({ go }) {
  ...
  return <main className="page">
    ...
    <div className="filter-layout">
      <aside className="tile filter-panel"><p className="eyebrow">RECOMMENDATION</p><h2>sample-api 기준</h2>{[['스킬 매칭', 70], ['신선도', 15], ['행동 신호', 10], ['인기', 5]].map(([l, v]) => <div className="bar-row" key={l}><b>{l}</b><ProgressBar value={v} /><span>{v}%</span></div>)}<Button variant="lime" onClick={() => go('projects')}>프로젝트 바꾸기</Button></aside>
