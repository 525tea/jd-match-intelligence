...
perf_jobs_count="$(printf '%s' "${jobs_response}" | jq -r '[.data[]? | select((.externalId // "") | startswith("perf-job-"))] | length')"
...
echo "jobs_perf_external_id_count=${perf_jobs_count}"

if (( perf_jobs_count < EXPECTED_MIN_RESULT_COUNT )); then
  echo "Expected /jobs to expose at least ${EXPECTED_MIN_RESULT_COUNT} performance fixture rows." >&2
  printf '%s\n' "${jobs_response}" >&2
  exit 1
fi
