SELECT
    source,
    COUNT(*) AS job_count,
    SUM(CASE WHEN raw_snapshot_key IS NOT NULL AND raw_snapshot_key <> '' THEN 1 ELSE 0 END) AS snapshotted_job_count,
    SUM(CASE WHEN raw_data IS NOT NULL AND raw_data <> '' THEN 1 ELSE 0 END) AS retained_raw_data_count,
    SUM(CASE WHEN raw_data IS NULL OR raw_data = '' THEN 1 ELSE 0 END) AS purged_raw_data_count,
    SUM(
            CASE
                WHEN raw_snapshot_key IS NULL OR raw_snapshot_key = ''
                    OR raw_snapshot_hash IS NULL OR raw_snapshot_hash = ''
                    OR raw_snapshot_size_bytes IS NULL OR raw_snapshot_size_bytes <= 0
                    OR raw_snapshot_storage_type IS NULL OR raw_snapshot_storage_type = ''
                    OR raw_snapshot_saved_at IS NULL
                    THEN 1
                ELSE 0
                END
    ) AS invalid_snapshot_metadata_count
FROM jobs
...
SELECT
    ...
    CASE
        WHEN raw_data IS NULL OR raw_data = '' THEN 'PURGED'
        ELSE 'RETAINED'
        END AS raw_data_state
FROM jobs
WHERE source IN ('JUMPIT', 'WANTED')
...
SELECT
    'RAW_SNAPSHOT_PURGE_BLOCKER' AS check_name,
    source,
    id,
    external_id,
    title,
    CASE
        ...
        WHEN raw_data IS NOT NULL AND raw_data <> '' THEN 'RAW_DATA_STILL_RETAINED'
        ELSE 'OK'
        END AS reason
FROM jobs
WHERE source IN ('JUMPIT', 'WANTED')
  AND (
    raw_snapshot_key IS NULL
        ...
        OR raw_snapshot_saved_at IS NULL
        OR (raw_data IS NOT NULL AND raw_data <> '')
    )
ORDER BY source, id
    LIMIT 50;
